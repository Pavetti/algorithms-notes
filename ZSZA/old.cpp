...
void praca_domowa()
{
    ...
    for (int i = 1; i < n - 1; ++i)
    {
        lewo[i] = max(a[i], lewo[i - 1] + a[i]);
    }
    for (int i = n - 2; i >= 1; --i)
    {
        prawo[i] = max(a[i], prawo[i + 1] + a[i]);
    }
    int wynik = INT_MIN;
    for (int i = 1; i < n - 1; ++i)
    {
        wynik = max(wynik, lewo[i - 1] + prawo[i + 1]);
    }
    cout << wynik << endl;
    // for (int i = 0; i < n; ++i) {
    //     cout << "lewo " << i  << ": "<< lewo[i] << " | prawo " << i  << ": " << prawo[i] << endl;
    // }
}
...
void bilet()
{
    ...
    {
        licznik += a[i];
        prefmax = max(licznik, prefmax);
    }
    licznik = 0;
    for (int i = n - 2; i >= 0; --i)
    {
        licznik += a[i];
        sufmax = max(licznik, sufmax);
    }
    int wynik = prefmax + sufmax;
    cout << wynik;
}
void odchudzanie()
{
...
void dwie_czesci()
{
    ...
    if (ile > 0)
    {
        ...
        if (count > n / 2)
        {
            ...
            for (int i = 0; i < n - 1; ++i)
            {
                if (prefixes[i] > (i + 1) / 2 && sufixes[i] > (n - (i + 1)) / 2)
                {
                    wynik++;
                }
            }
